...
const RegisterForm = ({id}) => {
  ...
  const handleSubmit = (data, memberIndex) => {
    ...
    if (currentMember < end) {
      ...
      axios.post(`/event/register/${id}`, flattenedData)
        .then(response => {
    ...
    <div style={{ 
      ...
      <SwitchTransition>
        <CSSTransition
          key={currentMember}
          timeout={500}
          classNames={'slide-appear'}  // Conditional transition
        >
          <TeamMemberForm
            key={currentMember}
            ...
            onBack={handleBack}  
            TeamNameRequired={(currentMember===1 &&end!=1) ? true : false}
          />
        </CSSTransition>
      </SwitchTransition>
    </div>
